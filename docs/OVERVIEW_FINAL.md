# AdAstra — Project Overview Final

> **Documentation status (2026-06-26 — final consolidation draft).** This file
> uses `docs/OVERVIEW.md` as the master, folds in still-current architecture
> and workflow material from `docs/OVERVIEWv2.md`, and spot-checks high-drift
> claims against the live source in `app/`, `database/`, `routes/`, `config/`,
> and `resources/`. Sections marked **[Accuracy Note]** call out places where
> prior copy had drifted from the code, or where current code is incomplete.
> See also **Recommendations & Remedies** and **Ambivalences for Review** at
...
- [Field Layouts](#field-layouts)
    ...
    - [Field Uniqueness Constraint](#field-uniqueness-constraint)
- [Status Groups and Statuses](#status-groups-and-statuses)
    ...
    - [StatusObserver — keeping status_is_public consistent](#statusobserver-keeping-statusispublic-consistent)
...
- [Entry Groups and Entry Types](#entry-groups-and-entry-types)
        - [Runtime resolution (corrected)](#runtime-resolution-corrected)
        - [Adding a new EntryType](#adding-a-new-entrytype)
...
- [Creating and Updating Entries](#creating-and-updating-entries)
    ...
    - [Using the Relationship Field](#using-the-relationship-field)
        - [Writing](#writing)
        - [Reading](#reading)
        - [Recursion with cycle detection](#recursion-with-cycle-detection)
        - [Raw pivot access](#raw-pivot-access)
...
- [Template and View Stack](#template-and-view-stack)
    ...
    - [View Namespaces and Includes](#view-namespaces-and-includes)
...
- [API Layer](#api-layer)
    ...
    - [API Resources and Current Limitations](#api-resources-and-current-limitations)
        - [API permission strings — current state](#api-permission-strings-current-state)
...
- [Known Gaps and Implementation Status](#known-gaps-and-implementation-status)
    - [Resolved since the original 2026-05-07 list](#resolved-since-the-original-2026-05-07-list)
    - [Still real](#still-real)
        - [Security & hardening (absorbed from `ALPHA_READINESS_REPORT.md`)](#security-hardening-absorbed-from-alphareadinessreportmd)
        - [Field Types layer (2026-05-26 audit)](#field-types-layer-2026-05-26-audit)
- [End-to-End Chain Traces](#end-to-end-chain-traces)
    - [Entry Create Chain](#entry-create-chain)
    - [Entry Update Chain](#entry-update-chain)
    - [Category Create Chain](#category-create-chain)
    - [Media Upload Chain](#media-upload-chain)
    - [User Create Chain](#user-create-chain)
...
- [Media Status Governance](#media-status-governance)
    ...
    - [Upload auto-assignment](#upload-auto-assignment)
    - [Validation](#validation)
    - [Admin UI](#admin-ui)
    - [Known limits](#known-limits)
- [HasStatus and the Status Sync Registry](#hasstatus-and-the-status-sync-registry)
    - [Contract](#contract)
    - [What the observer does](#what-the-observer-does)
...
- [Recommendations & Remedies](#recommendations-remedies)
    - [R-1 — Missing `_fields/html.twig` partial (High)](#r-1-missing-fieldshtmltwig-partial-high)
    - [R-2 — Broken `route('settings.user')` redirect (Resolved 2026-05-26)](#r-2-broken-routesettingsuser-redirect-resolved-2026-05-26)
    - [R-3 — `Api\v1\User::index` permission typo (High)](#r-3-apiv1userindex-permission-typo-high)
    - [R-4 — Duplicate user-settings controller, one unrouted (Resolved 2026-05-26)](#r-4-duplicate-user-settings-controller-one-unrouted-resolved-2026-05-26)
    - [R-5 — `Admin\Settings\Domain::index` unrouted (Low)](#r-5-adminsettingsdomainindex-unrouted-low)
    - [R-6 — `Api\v1\Account` placeholder stubs (Medium)](#r-6-apiv1account-placeholder-stubs-medium)
    - [R-7 — `EntryType.max_depth` / `allowed_parent_types` unenforced (Medium)](#r-7-entrytypemaxdepth-allowedparenttypes-unenforced-medium)
    - [R-8 — `app:refresh-tokens` is implemented but unscheduled (Low)](#r-8-apprefresh-tokens-is-implemented-but-unscheduled-low)
    - [R-9 — `site.templates.base_path` / `not_found_template` unread (Low)](#r-9-sitetemplatesbasepath-notfoundtemplate-unread-low)
    - [R-10 — `Admin\User\Layout` is mostly empty (Low)](#r-10-adminuserlayout-is-mostly-empty-low)
    - [R-11 — `Admin\Index` is dead code (Low)](#r-11-adminindex-is-dead-code-low)
    - [R-12 — `Admin\Field::index` returns 404 (Low)](#r-12-adminfieldindex-returns-404-low)
    - [R-13 — `Admin\Role::show()` is empty (Low)](#r-13-adminroleshow-is-empty-low)
    - [R-14 — `Admin\Dashboard` does raw SQL aggregation (Low)](#r-14-admindashboard-does-raw-sql-aggregation-low)
    - [R-15 — Tree-create runs after the entry transaction commits (Medium)](#r-15-tree-create-runs-after-the-entry-transaction-commits-medium)
    - [R-16 — Two flash keys (`success` vs `status`) (Low)](#r-16-two-flash-keys-success-vs-status-low)
    - [Security & Hardening (Alpha review pass)](#security-hardening-alpha-review-pass)
    - [R-17 — `UpdateUserPassword` skips the current-password check (Critical)](#r-17-updateuserpassword-skips-the-current-password-check-critical)
    - [R-18 — Role assignment hardening: service-layer defence-in-depth (Critical)](#r-18-role-assignment-hardening-service-layer-defence-in-depth-critical)
    - [R-19 — `LogRequestResponse` never writes `response_payload` (Resolved — by decision)](#r-19-logrequestresponse-never-writes-responsepayload-resolved--by-decision)
    - [R-20 — Personal access token flashed in URL/session (High)](#r-20-personal-access-token-flashed-in-urlsession-high)
    - [R-21 — `EntryTypeRegistry::resolveByHandle` is group-blind (High, defence-in-depth)](#r-21-entrytyperegistryresolvebyhandle-is-group-blind-high-defence-in-depth)
    - [R-22 — `User::$fillable` exposes status columns to mass-assignment (High)](#r-22-userfillable-exposes-status-columns-to-mass-assignment-high)
    - [R-23 — CORS is wide-open (`*` origins, `*` methods, `*` headers) (High)](#r-23-cors-is-wide-open-origins-methods-headers-high)
    - [R-24 — API mutator endpoints rely solely on FormRequest authorize (Medium)](#r-24-api-mutator-endpoints-rely-solely-on-formrequest-authorize-medium)
    - [R-25 — `UserService::updateToken()` accepts arbitrary fields (Medium)](#r-25-userserviceupdatetoken-accepts-arbitrary-fields-medium)
    - [R-26 — `users.default_status` / `social_default_status` accept post-creation statuses (Medium)](#r-26-usersdefaultstatus-socialdefaultstatus-accept-post-creation-statuses-medium)
    - [R-27 — Media library handle is not slug-restricted (Low)](#r-27-media-library-handle-is-not-slug-restricted-low)
    - [R-28 — SVG / HTML uploads on a public disk are stored-XSS vectors (Low, conditional)](#r-28-svg-html-uploads-on-a-public-disk-are-stored-xss-vectors-low-conditional)
    - [R-29 — `BotBlockRequest` only checks `POST` (Low)](#r-29-botblockrequest-only-checks-post-low)
    - [R-30 — `Entry::$fillable` exposes `created_by_user_id` (Low)](#r-30-entryfillable-exposes-createdbyuserid-low)
    - [Field Types layer findings (2026-05-26 audit)](#field-types-layer-findings-2026-05-26-audit)
    - [R-31 — `AbstractField::validate()` is unreachable infrastructure (Medium)](#r-31-abstractfieldvalidate-is-unreachable-infrastructure-medium)
    - [R-32 — `Telephone::$rules` references unregistered `'telephone'` validator (Critical)](#r-32-telephonerules-references-unregistered-telephone-validator-critical)
    - [R-33 — `EmailAddress::$rules` is empty — no format validation (Medium)](#r-33-emailaddressrules-is-empty-no-format-validation-medium)
    - [R-34 — Field-type setting forms advertise unenforced constraints (Low)](#r-34-field-type-setting-forms-advertise-unenforced-constraints-low)
    - [R-35 — `ValidatesAgainstOptions::renderOrphanedValue()` is dead helper (Low)](#r-35-validatesagainstoptionsrenderorphanedvalue-is-dead-helper-low)
...
- [Ambivalences for Review](#ambivalences-for-review)
    - [A-1 — EntryTypeRegistry resolves by handle, not by group](#a-1-entrytyperegistry-resolves-by-handle-not-by-group)
    - [A-2 — Two near-identical user-settings stacks (Resolved 2026-05-26)](#a-2-two-near-identical-user-settings-stacks-resolved-2026-05-26)
    - [A-3 — `Admin\Field::index` is intentional 404 vs scaffolding mistake](#a-3-adminfieldindex-is-intentional-404-vs-scaffolding-mistake)
    - [A-4 — `MediaPicker` route and controller — undocumented (Resolved 2026-05-26)](#a-4-mediapicker-route-and-controller-undocumented-resolved-2026-05-26)
    - [A-5 — `Money` field: storage convention (Resolved 2026-05-26)](#a-5-money-field-storage-convention-resolved-2026-05-26)
    - [A-6 — `Entry::redirect_url` validation vs `EntryTree::redirect_url` storage](#a-6-entryredirecturl-validation-vs-entrytreeredirecturl-storage)
    - [A-7 — `MediaResource` not yet built](#a-7-mediaresource-not-yet-built)
    - [A-8 — `Entry` lacks `SoftDeletes` while `Media` has it](#a-8-entry-lacks-softdeletes-while-media-has-it)
    - [A-9 — `default_schema_type` on `entry_types` and `schema_type` on `entries`](#a-9-defaultschematype-on-entrytypes-and-schematype-on-entries)
    - [A-10 — Categorical handle uniqueness across groups](#a-10-categorical-handle-uniqueness-across-groups)
- [Accuracy Notes Log](#accuracy-notes-log)
    - [Items absorbed from ALPHA_READINESS_REPORT.md (2026-05-26)](#items-absorbed-from-alphareadinessreportmd-2026-05-26)
...
| `routes/api.php` | `/api/v1` | `auth:sanctum`; per-resource `LogRequestResponse` | REST API |
| `routes/web.php` | none | `web` | Public site catch-all plus auth/OAuth routes |
| `routes/console.php` | n/a | n/a | Scheduled jobs and console commands |

Public site requests fall through to `App\Http\Controllers\Site::show()`,
which delegates to `App\Services\SiteRouting\SiteRouter::render($uri)`.
The router tries the drivers in `config('site.routing.priority')` in order;
the default order is `entry_tree`, then `template`.

Admin requests first pass the `auth` route middleware, then the base
`Admin\Controller` constructor checks `access admin`. Write actions usually
receive a FormRequest whose `authorize()` method checks the more specific
Spatie permission for that operation.

API requests use Sanctum. Each API resource route is wrapped with
`LogRequestResponse`; read and delete actions also tend to perform explicit
controller-level `$this->can(...)` checks. Mutator actions often rely on the
shared FormRequest authorization path, which is why R-24 below recommends a
consistent controller-level defence-in-depth pass for API writes.
...
environment. `routes/console.php` schedules `PruneApiLogs` daily at 02:00 and
`PurgeDeletedMedia` daily at 03:00; both require the scheduler cron to be
active.
...
The `app:refresh-tokens` console command is implemented and refreshes expiring
OAuth tokens through `TokenRefreshService`. It is not currently scheduled in
`routes/console.php`, so automatic refresh requires either a schedule entry or
an external caller.
...
TwigBridge is the view layer for admin templates, and public site templates
also render through Laravel's view system using the `templates::` namespace.
The app registers two namespaces in `AppServiceProvider::boot()`:
...
Admin templates live under `resources/views/admin/` and are referenced as
`admin::...` from controllers through the base admin view helper. Public site
templates live under `resources/templates/` and are referenced as
`templates::...` by the route drivers.

### View Namespaces and Includes

Field partials are currently loaded from `resources/views/_fields/` through
the default Laravel view path; no `_fields` namespace is registered. That is
fine for the current layout, but moving field partials out of the root view
path later will require an explicit namespace.

The admin layout is `resources/views/admin/_inc/_layout.twig`. Some older
admin include files remain in the tree even though the layout no longer uses
them; treat those as cleanup candidates rather than active structure.

### Site Templates

`TemplateRouteDriver` resolves public template routes by convention:

- `/` uses `config('site.templates.default_template')`.
- `/{group}` tries `templates::{group}.index`.
- `/{group}/{template}` first tries `templates::{group}.{template}`.
- If that action template does not exist, it falls back to
  `templates::{group}.entry` with the second segment exposed as `handle`.

The driver rejects reserved first segments such as `admin`, `api`, `login`,
`storage`, and `vendor`, and refuses to render any `admin::` view through the
public catch-all.

### Vite Asset Pipeline

The project uses Vite 7. Local development can be started with
`composer run dev`, which runs Laravel's server, the queue listener, and Vite
together. Production builds should use `npm run build` after dependencies are
installed.
...
`routes/console.php` schedules `App\Jobs\PruneApiLogs` daily at 02:00, and
`PurgeDeletedMedia` runs daily at 03:00 for the media cleanup path.
...
Admin controllers extend `App\Http\Controllers\Admin\Controller`, which checks
`access admin` in its constructor. Most admin writes are guarded again by a
FormRequest with a resource-specific permission check. API controllers extend
`App\Http\Controllers\Api\Controller`; they return API resources or JSON
responses and share the `auth:sanctum` route group.

The preferred controller shape is deliberately thin: validate with a
FormRequest, delegate mutations to an action/service/facade, and redirect or
return a resource. Controllers should not write directly to core `Entry`
attributes; the Entry lifecycle belongs to `EntryService` and
`EntryRepository`.

### Admin Controllers — Catalog

The admin route map is defined in `routes/admin.php`. The active controller
families are:
...
API controllers live under `App\Http\Controllers\Api\v1\` and are registered
in `routes/api.php` under `/api/v1`:

| Resource | Controller | Current shape |
|---|---|---|
| Users | `Api\v1\User` | Full resource controller; `index()` still has the singular `read user` typo. |
| Account | `Api\v1\Account` | Placeholder account methods; see R-6. |
...
Services coordinate business operations and should be the preferred home for
transaction orchestration, lifecycle decisions, and cross-model writes.
Controllers and commands may use facades for convenience; services and
repositories should prefer constructor injection or explicit dependencies.

Important service entry points include `EntryService`, `EntryTypeService`,
`EntryGroupService`, `CategoryService`, `UserService`, `EntryAuthorService`,
`MediaStorageService`, and `Settings`.

### Repositories Layer

Repositories own persistence details. `EntryRepository` is intentionally more
specialised than a generic fieldable repository because entries combine core
attributes, statuses, authors, categories, scalar field values, relationship
field values, and tree-adjacent state. For new Entry behaviour, keep the
three-layer contract intact: `AbstractEntryType` subclass, `EntryRepository`,
and `EntryService`.

### Actions Layer

Action classes sit between controllers and services for admin workflows that
need request-shaped orchestration, redirects, or small multi-step routines.
They should stay thin and should not become a second persistence layer.

### FormRequests Layer

FormRequests own two things: authorization and input shape. For fieldable
models, request rules combine static model rules with dynamic schema rules
from the relevant field layout. Entry requests also validate status handles,
author eligibility, category membership, tree options, and redirect metadata.

### Observers, Events, and Listeners

Registered observers include `StatusObserver`, `EntryTreeObserver`, and
`FieldValueObserver`. User status and lock changes dispatch events that the
`WriteUserStatusLog` listener records into `user_status_logs`.

### Schema Macros

`AppServiceProvider::register()` defines `Blueprint::statusColumns()`, the
shared denormalized status triple used by status-aware models: `status_id`,
`status_handle`, and `status_is_public`.

### Transactional Behaviour

Entry creation and update use repository transactions for core entry writes,
status assignment, authors, categories, and field values. Some related work,
notably entry-tree creation, still happens outside the core transaction; R-15
tracks that risk.
...
- **R-8**: `app:refresh-tokens` is implemented, but no schedule entry
...
- **R-19 (Resolved — by decision)**: Response bodies are intentionally
  not persisted in `api_logs`. The dead `summarizeResponse()` method
  was removed rather than wired up; no `response_payload` column was
  ever added.
...
### R-8 — `app:refresh-tokens` is implemented but unscheduled (Low)

**Symptom.** `RefreshTokens::handle()` now queries active expiring OAuth
tokens and calls `TokenRefreshService::tryRefresh()` for each. The remaining
gap is operational: `routes/console.php` does not schedule
`app:refresh-tokens`, so token refresh only runs when invoked manually.

**Fix.** Add a schedule entry in `routes/console.php` if automatic OAuth
refresh is required, for example every 15 minutes with an expiry window that
matches provider tolerance.
...
### R-19 — `LogRequestResponse` never writes `response_payload` (Resolved — by decision)

**Status (2026-06-26).** Closed. Product decided **not** to persist response
bodies in `api_logs` at all — the security/storage cost (response bodies can
carry sensitive data the redaction list doesn't anticipate, plus unbounded
storage growth) outweighed the postmortem convenience. The dead
`LogRequestResponse::summarizeResponse()` method has been **deleted** rather
than wired up.

**Original symptom (now moot).** The `response_payload` column was never
actually declared in the schema — `database/migrations/2025_11_07_174041_create_api_log_table.php`
and `app/Models/ApiLog.php`'s `$fillable`/`$casts` never included it. The
original claim that "the column is declared in the schema" was inaccurate.
`ApiLog::create([...])` only ever wrote `request_payload`, `request_headers`,
`response_headers`, `response_status_code` — by design, not by oversight.

**Resulting tradeoff.** Postmortems on production 500s will not have the
rendered response body available — only the request, status code, and
timing. If a future need arises, capture response bodies into a separate,
shorter-retention store rather than reintroducing them into `api_logs`.
...
The `docs/` directory contains several forward-looking plans. Treat this
section as a coordination map, not implementation approval.

### Plan Documents in `docs/`

| File | Status / role |
|---|---|
| `ACTION_PLAN.md` | Meta-triage and sequencing document. Its older overview-gap notes have been superseded by this final overview pass. |
| `MEDIA_LAYER_OVERVIEW.md` | Authoritative current-state reference for the native Media layer. |
| `media-status-implementation-plan.md` | Implemented design record for media status governance; keep for rationale. |
| `TenantPlan.md` | Current multi-tenant foundation plan. |
| `TenantModel.md` | Earlier tenant exploration; mostly superseded by `TenantPlan.md`. |
| `MULTILINGUAL_PLAN.md` | Planned content translation/localization work. |
| `SEARCH_PLAN_V2.md` | Current search plan; use this over `SEARCH_PLAN.md`. |
| `SEARCH_PLAN.md` | Superseded V1 search plan. Mark or archive to avoid confusion. |
| `SEO_SCHEMA_PLAN.md` | Planned schema.org JSON-LD work. |
| `DISCUSSION_LAYER_PLAN.md` | Planned comments/reviews/discussion layer. |
| `SHOP_PLAN.md` | Planned e-commerce module, but currently conflicts with `TODOS.md` item 12. |
| `Core50.md` | Product plan built on top of the platform, not core platform work. |
| `BASICS.md` | Contributor orientation; field-type list is older than the current code. |
| `ENTRY_LAYER_OVERVIEW.md` | Detailed entry-layer reference and audit. |
| `api-documentation-migration-plan.md` | OpenAPI migration plan; refresh against current API attributes before acting. |
| `TODOS.md` | Active small-task list and policy reminders. |

### Cross-Plan Contradictions

Items to reconcile before executing adjacent plans:

1. `mithra62` vs `mithra72` appears in some older module diagrams. Live code
   and composer use `mithra62`.
2. Older plans still mention Spatie MediaLibrary. The native Media layer is
   authoritative now.
3. `SEARCH_PLAN.md` and `SEARCH_PLAN_V2.md` coexist; V2 is the current plan.
4. `SHOP_PLAN.md` assumes Shop remains in composer, while `TODOS.md` says to
   remove it until Shop work begins.
5. `field_layout_tab_elements` is a coordination hotspot: Search V2 wants
   `is_searchable` / `search_weight`, SEO Schema wants `schema_property`, and
   TenantPlan wants `tenant_id`.
6. `BASICS.md` omits several built-in field types and should be refreshed
   after this overview is adopted.
...
- C-3 → R-18 (role-assignment privilege escalation, service-layer
  defence-in-depth).
- H-2 → R-19 (`LogRequestResponse` response-body persistence — resolved
  by decision not to store response bodies; see R-19).
...
- I-4 → R-8 (`app:refresh-tokens` now implemented; scheduling remains open).
